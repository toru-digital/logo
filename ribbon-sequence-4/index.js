...
const project = getProject("Logo animation");
const sheet = project.sheet("Animated logo");
...
const logoGroupObj = sheet.object('LogoGroup', {
  // add opacity 
	rotation: types.compound({
	  x: types.number(logoGroup.rotation.x, { range: [-2, 2] }),
	  y: types.number(logoGroup.rotation.y, { range: [-2, 2] }),
	  z: types.number(logoGroup.rotation.z, { range: [-2, 2] }),
	}),
	scale: types.compound({
	  x: types.number(logoGroup.scale.x, { range: [-2, 2] }),
	  y: types.number(logoGroup.scale.y, { range: [-2, 2] }),
	  z: types.number(logoGroup.scale.z, { range: [-2, 2] }),
	}),
	position: types.compound({
	  x: types.number(logoGroup.position.x, { range: [-500, 500] }),
	  y: types.number(logoGroup.position.y, { range: [-500, 500] }),
	  z: types.number(logoGroup.position.z, { range: [-500, 500] }),
	}),
  });
  
  logoGroupObj.onValuesChange((values) => {
	const { x, y, z } = values.rotation;
	logoGroup.rotation.set(x * Math.PI, y * Math.PI, z * Math.PI);
  
	const { x: sx, y: sy, z: sz } = values.scale;
	logoGroup.scale.set(sx, sy, sz);
  
	const { x: px, y: py, z: pz } = values.position;
	logoGroup.position.set(px, py, pz);
  
	buildLogo(); // Update the logo shapes after adjustments
  });
